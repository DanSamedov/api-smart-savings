...
from app.api.dependencies import (get_current_user, get_ims_service, get_redis,
                                  require_savebuddy_consent)
from app.core.middleware.rate_limiter import limiter
from app.core.utils.exceptions import CustomException
from app.core.utils.response import standard_response
from app.modules.ims.schemas import (CancelResponse, ChatHistoryResponse,
                                     ConfirmResponse,
                                     ConfirmTransactionRequest,
                                     DraftTransaction, IMSInputSchema,
                                     InterpretResponse, ScheduledListResponse,
...
@router.post(
    "/interpret", status_code=status.HTTP_200_OK, response_model=InterpretResponse
)
...
async def interpret_prompt(
    ...
    _consent: bool = Depends(require_savebuddy_consent),
) -> dict[str, Any]:
    """
...
@router.post(
    "/confirm", status_code=status.HTTP_201_CREATED, response_model=ConfirmResponse
)
@limiter.limit("5/minute")
async def confirm_transaction(
    ...
    try:
        ...
        response_data = ScheduledTransactionResponse(
            ...
            projected_dates=[scheduled_tx.next_run_at]  # First date for now
...
@router.get(
    "/scheduled", status_code=status.HTTP_200_OK, response_model=ScheduledListResponse
)
...
@router.get(
    "/history", status_code=status.HTTP_200_OK, response_model=ChatHistoryResponse
)
@limiter.limit("10/minute")

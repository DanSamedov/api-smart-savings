...
class UserRepository:
    ...
    async def get_transaction_count_last_n_days(
        ...
        stmt = select(func.count(Transaction.id)).where(
            and_(Transaction.owner_id == user_id, Transaction.created_at >= cutoff_date)
        )
        result = await self.db.execute(stmt)
    ...
    async def get_total_group_contributions(self, user_id: UUID) -> float:
        ...
        stmt = (
            select(func.coalesce(func.sum(GroupMember.contributed_amount), 0))

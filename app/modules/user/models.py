...
class User(UserBase, table=True):
    ...
    preferred_currency: Currency = Field(
        sa_column=Column(
            Currency.sa_enum(), nullable=False, server_default=Currency.EUR.value
        )
    )
    preferred_language: Optional[str] = None

    gdpr_requests: list["GDPRRequest"] = Relationship(
    ...
    transactions: list["Transaction"] = Relationship(
        back_populates="owner", cascade_delete=False
    )
    ...
    scheduled_transactions: list["ScheduledTransaction"] = Relationship(
        back_populates="user"
    )

...
from app.modules.shared.enums import (NotificationType, TransactionStatus,
                                      TransactionType)
...
class WalletService:
    ...
    async def _send_wallet_io_notification(
        self, current_user, wallet, transaction_request, transaction, background_tasks
    ):
        try:
            full_name = (
                current_user.full_name if current_user.full_name is not None else ""
            )
            currency = (
                transaction_request.currency.value
                if transaction_request.currency is not None
            ...
            context = {
                ...
                "transaction_amount": f"{float(transaction.amount):,.2f}".rstrip(
                    "0"
                ).rstrip("."),
                "transaction_date": transform_time(transaction.created_at),
                "updated_balance": f"{float(wallet.total_balance):,.2f}".rstrip(
                    "0"
                ).rstrip("."),
                "currency": currency,
            }
    ...
    async def get_balance(self, redis: Redis, current_user: User) -> dict[str, Any]:
        ...
        async def fetch_balance():
            ...
            if not wallet:
                raise CustomException.e404_not_found(
    ...
    async def get_transactions(
        ...
        cache_key = (
            f"wallet_transactions:{current_user.id}:page:{page}:size:{page_size}"
        )

        async def fetch_transactions():
            total_transactions = (
                await self.transaction_repo.get_user_transactions_count(current_user.id)
            )
            ...
            if total_pages and page > total_pages:
                ...
                transactions = (
                    await self.transaction_repo.get_user_transactions_paginated(
    ...
    async def deposit(
        ...
        if (
            transaction_request.amount < min_deposit
            or transaction_request.amount > max_deposit
        ):
            raise CustomException.e400_bad_request(
                f"Transaction amount must be between {min_deposit} and {max_deposit} {current_user.preferred_currency.value}"
            )
        ...
        transaction = await self._record_transaction(
            ...
            tx_type=TransactionType.WALLET_DEPOSIT,
        )
        ...
        await self._send_wallet_io_notification(
            current_user, wallet, transaction_request, transaction, background_tasks
        )
    ...
    async def withdraw(
        ...
        if (
            transaction_request.amount < min_withdrawal
            or transaction_request.amount > max_withdrawal
        ):
            raise CustomException.e400_bad_request(
                f"Transaction amount must be between {min_withdrawal} and {max_withdrawal} {current_user.preferred_currency.value}"
            )
        ...
        transaction = await self._record_transaction(
            ...
            user=current_user,
            amount=transaction_request.amount,
            tx_type=TransactionType.WALLET_WITHDRAWAL,
        )

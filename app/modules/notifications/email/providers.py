...
class ResendProvider(EmailProvider):
    ...
    def __init__(self):
        if not settings.RESEND_API_KEY:
            logger.warning(
                "RESEND_API_KEY is not set. Email sending via Resend provider will fail."
            )
    ...
    async def send_email(
        ...
        try:
                    ...
                    formatted_attachments.append(
                        {
                            ...
                            "content": list(
                                content
                            ),  # Resend expects a list of integers (bytes)
                        }
                    )
            ...
            await loop.run_in_executor(None, partial(resend.Emails.send, email_params))
            logger.info(
                f"Email '{mask_email(subject)}' sent to {mask_email(recipients[0])} via Resend."
            )

        except Exception as e:
            logger.exception(
                f"Failed to send email '{mask_data(subject)}' to {mask_email(recipients[0])} via Resend: {e}"
            )
            raise e
...
class SMTPProvider(EmailProvider):
    ...
    def __init__(self):
        if not settings.SMTP_HOST or not settings.SMTP_USERNAME:
            logger.warning(
                "SMTP settings are incomplete. Email sending via SMTP may fail."
            )
    ...
    async def send_email(
        ...
        try:
            ...
            logger.info(
                f"Email '{mask_data(subject)}' sent to {mask_email(recipients[0])} via SMTP."
            )

        except Exception as e:
            logger.exception(
                f"Failed to send email '{mask_data(subject)}' to {mask_email(recipients[0])} via SMTP: {e}"
            )
            raise e
...
class EmailProviderFactory:
    ...
    def get_provider() -> EmailProvider:
        ...
        if provider_type == "smtp":
            ...
            logger.warning(
                f"Unknown email provider '{provider_type}', defaulting to Resend."
            )
            return ResendProvider()

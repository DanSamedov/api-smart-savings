...
from app.modules.ims.schemas import (ChatHistoryItem,
                                     ConfirmTransactionRequest,
                                     DraftTransaction, IMSContextSchema,
                                     InterpretationData,
                                     ProjectionScheduleItem)
from app.modules.shared.enums import (Currency, DestinationType,
...
class ProjectionService:
    ...
    def get_projection_schedule(
        ...
        """
        dates: List[datetime] = []
        current = ProjectionService._calculate_first_run(
            start_date, frequency, day_of_week
        )

        if frequency == TransactionFrequency.ONCE:
            return (
                [current]
                if ProjectionService._is_within_range(current, end_date)
                else []
            )
    ...
    @staticmethod
    def _get_next_date(
        current: datetime, frequency: TransactionFrequency, day_of_week: Optional[int]
    ) -> datetime:
    ...
    def create_draft(
        ...
        if draft.amount is None:
            ...
            validation_messages.append(
                "Please provide the amount for this transaction."
            )
...
class IMSService:
    ...
    async def interpret_prompt(
        ...
        # Save prompt action
        await self.ims_repo.create_action(
            IMSAction(
                user_id=current_user.id,
                user_prompt=prompt,
                intent=interpretation.intent.value
                if hasattr(interpretation.intent, "value")
                else str(interpretation.intent),
        ...
        draft = ProjectionService.create_draft(
            interpretation, user_groups=user_groups, user_goals=user_goals
        )

        logger.info(
            f"Created draft transaction for user {current_user.id}: {draft.validation_status}"
        )
    ...
    async def _call_nlp_service(self, context: IMSContextSchema) -> InterpretationData:
        ...
        try:
            ...
            raise ValueError(
                f"Failed to connect to interpretation service at {NLP_SERVICE_URL}: {e}"
            )
    ...
    async def confirm_transaction(
        ...
        if request.destination_type == DestinationType.GROUP:
            ...
            found = next(
                (g for g in groups if g.name.lower() == request.group_name.lower()),
                None,
            )
            ...
            found = next(
                (g for g in goals if g.name.lower() == request.goal_name.lower()), None
            )
            ...
            if found:
                ...
                raise ValueError(
                    f"Goal '{request.goal_name}' not found. Please create it first."
                )
    ...
    async def get_chat_history(self, current_user: User) -> List[ChatHistoryItem]:
        ...
        transactions = await self.ims_repo.get_scheduled_transactions_by_user(
            current_user.id
        )

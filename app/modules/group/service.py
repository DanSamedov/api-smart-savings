...
from app.modules.group.models import GroupBase, GroupMember
...
class GroupService:
    ...
    async def create_group(self, group_in: GroupBase, current_user: User):
        ...
        if not current_user.stag:
            raise CustomException.bad_request(
                "You must set your stag before creating a group."
            )
    ...
    async def get_group(self, group_id: uuid.UUID, current_user: User):
        ...
        is_member = any(
            str(member.user_id) == str(current_user.id) for member in group.members
        )
        if not is_member:
    ...
    async def update_group_settings(
        self, group_id: uuid.UUID, group_in: GroupUpdate, current_user: User
    ):
        """
    ...
    async def delete_group(self, group_id: uuid.UUID, current_user: User):
        ...
        raise CustomException.not_found(
            detail="Group not found or could not be deleted"
        )
    ...
    async def get_group_members(self, group_id: uuid.UUID, current_user: User):
        ...
        members_with_details = await self.group_repo.get_group_members_with_details(
            group_id
        )
    ...
    async def add_group_member(
        ...
        if group.is_solo:
            raise CustomException.bad_request(
                detail="Cannot add members to a solo group"
            )
        ...
        if any(member.user_id == user_to_add.id for member in members):
            raise CustomException.bad_request(
                detail=f"{member_in.stag} is already a member"
            )

        max_members = settings.MAX_GROUP_MEMBERS or 7
        if len(members) >= max_members:
            raise CustomException.bad_request(
                detail=f"Groups are limited to {max_members} members maximum"
            )

        # Cooldown validation
        removed_member = await self.group_repo.get_removed_member(
            group_id, user_to_add.id
        )
        if removed_member:
            cooldown_days = settings.REMOVE_MEMBER_COOLDOWN_DAYS
            if removed_member.removed_at + timedelta(days=cooldown_days) > datetime.now(
                timezone.utc
            ):
                raise CustomException.bad_request(
        ...
        if new_member:
            ...
            await self.notification_manager.schedule(
                ...
                context={
                    ...
                    "group_current_balance": f"{float(group.current_balance):,.2f}".rstrip(
                        "0"
                    ).rstrip(
                        "."
                    ),
                    "group_target_balance": f"{float(group.target_balance):,.2f}".rstrip(
                        "0"
                    ).rstrip(
                        "."
                    ),
    ...
    async def remove_group_member(
        ...
        members = await self.group_repo.get_group_members(group_id)
        member_to_check = next(
            (m for m in members if m.user_id == member_in.user_id), None
        )
        ...
        if (
            member_to_remove
            and member_to_remove.contributed_amount
            > settings.MIN_GROUP_THRESHOLD_AMOUNT
        ):
            raise CustomException.bad_request(
        ...
        removed = await self.group_repo.remove_member_from_group(
            group_id, member_in.user_id
        )
        if removed:
            ...
            if removed_member_user:
                ...
                await self.notification_manager.schedule(
                    ...
                    context={
                        "member_name": removed_member_user.full_name
                        or removed_member_user.email,
                        "group_name": group.name,
                        "group_admin_name": current_user.full_name
    ...
    async def contribute_to_group(
        ...
        try:
            ...
            await self.group_repo.update_member_contribution(
                group_id, current_user.id, amount_to_contribute
            )

            # 5. Create group transaction message
            await self.group_repo.create_group_transaction_message(
                group_id, current_user.id, amount_to_contribute, tx_type
            )
        ...
        current_member = next(
            (m for m in updated_members if str(m.user_id) == str(current_user.id)), None
        )
        ...
        if admin_members:
            ...
            contributor_context = {
                ...
                "contribution_amount": f"{float(amount_to_contribute):,.2f}".rstrip(
                    "0"
                ).rstrip("."),
                "currency": currency,
                "group_current_balance": f"{float(updated_group.current_balance):,.2f}".rstrip(
                    "0"
                ).rstrip(
                    "."
                ),
                "group_target_balance": f"{float(updated_group.target_balance):,.2f}".rstrip(
                    "0"
                ).rstrip(
                    "."
                ),
                "member_total_contributed": f"{float(current_member.contributed_amount):,.2f}".rstrip(
                    "0"
                ).rstrip(
                    "."
                )
            ...
            if updated_group.target_balance > 0:
                current_percentage = (
                    float(updated_group.current_balance)
                    / float(updated_group.target_balance)
                ) * 100
                previous_percentage = (
                    float(previous_balance) / float(updated_group.target_balance)
                ) * 100
        ...
        return {
            ...
            "group": {
                "current_balance": float(updated_group.current_balance)
                if updated_group
                else 0.0,
                "target_balance": float(updated_group.target_balance)
                if updated_group
                else 0.0,
            },
            "member": {
                "total_contributed": float(current_member.contributed_amount)
                if current_member
                else 0.0,
            },
        }
    ...
    async def remove_contribution(
        ...
        try:
            # Determine transaction type based on group type
            tx_type = (
                TransactionType.INDIVIDUAL_SAVINGS_WITHDRAWAL
                if group.is_solo
                else TransactionType.GROUP_SAVINGS_WITHDRAWAL
            )
            ...
            # 4. Update member contribution
            await self.group_repo.update_member_contribution(
                group_id, current_user.id, -amount_to_withdraw
            )

            # 5. Create group transaction message
            await self.group_repo.create_group_transaction_message(
                group_id, current_user.id, amount_to_withdraw, tx_type
            )
        ...
        await self.notification_manager.schedule(
            ...
            context={
                ...
                "group_name": updated_group.name,
                "withdrawal_amount": f"{float(amount_to_withdraw):,.2f}".rstrip(
                    "0"
                ).rstrip("."),
                "currency": currency,
                "group_current_balance": f"{float(updated_group.current_balance):,.2f}".rstrip(

...
@pytest.fixture
def group_service(
    mock_group_repo, mock_user_repo, mock_wallet_repo, mock_notification_manager
):
    return GroupService(
        mock_group_repo, mock_user_repo, mock_wallet_repo, mock_notification_manager
    )
...
@pytest.mark.asyncio
async def test_add_member_to_solo_group_fails(
    group_service, mock_group_repo, mock_user_repo, current_user, background_tasks
):
    group_id = uuid.uuid4()

    # Mock solo group
    mock_group_repo.get_group_by_id.return_value = Group(
        id=group_id, name="Solo Group", target_balance=1000, is_solo=True
    )
...
@pytest.mark.asyncio
async def test_contribute_to_solo_group_with_one_member_success(
    group_service,
    mock_group_repo,
    mock_wallet_repo,
    current_user,
    ...
    member = GroupMember(
        group_id=group_id, user_id=current_user.id, contributed_amount=0
    )
...
@pytest.mark.asyncio
async def test_contribute_to_normal_group_with_one_member_fails(
    group_service,
    mock_group_repo,
    mock_wallet_repo,
    current_user,
...
@pytest.mark.asyncio
async def test_solo_group_auto_override_admin_approval_on_create(
    group_service, mock_group_repo, current_user
):
...
@pytest.mark.asyncio
async def test_solo_group_auto_override_admin_approval_on_update(
    group_service, mock_group_repo, current_user
):
    ...
    result = await group_service.update_group_settings(
        group_id, group_update, current_user
    )
...
@pytest.mark.asyncio
async def test_change_is_solo_from_false_to_true_fails(
    group_service, mock_group_repo, current_user
):
    ...
    mock_group_repo.get_group_by_id.return_value = Group(
        id=group_id, name="Squad Group", target_balance=1000, is_solo=False
    )
    mock_group_repo.is_user_admin.return_value = True
...
@pytest.mark.asyncio
async def test_change_is_solo_from_true_to_false_fails(
    group_service, mock_group_repo, current_user
):
...
@pytest.mark.asyncio
async def test_squad_group_can_toggle_admin_approval(
    group_service, mock_group_repo, current_user
):

...
def mock_settings(monkeypatch):
    monkeypatch.setattr(
        "app.modules.group.service.settings.REMOVE_MEMBER_COOLDOWN_DAYS", 7
    )
...
@pytest.mark.asyncio
async def test_add_group_member_max_limit(
    group_service,
    mock_group_repo,
    mock_user_repo,
    current_user,
    ...
    mock_group_repo.get_group_by_id.return_value = Group(
        id=group_id, name="Test Group", target_balance=1000
    )
    mock_group_repo.is_user_admin.return_value = True

    # Mock user resolution
    mock_user_repo.get_by_stag.return_value = User(
        id=user_to_add_id, email=email_to_add, stag=stag_to_add
    )
...
@pytest.mark.asyncio
async def test_add_group_member_cooldown(
    group_service,
    mock_group_repo,
    mock_user_repo,
    current_user,
    ...
    # Mock user resolution
    mock_user_repo.get_by_stag.return_value = User(
        id=user_id_to_add, email=email_to_add, stag=stag_to_add
    )
    ...
    mock_group_repo.get_removed_member.return_value = RemovedGroupMember(
        group_id=group_id, user_id=user_id_to_add, removed_at=removed_at
    )
...
@pytest.mark.asyncio
async def test_remove_group_member_with_contributions(
    group_service,
    mock_group_repo,
    mock_user_repo,
    current_user,
    ...
    monkeypatch.setattr(
        "app.modules.group.service.settings.MIN_GROUP_THRESHOLD_AMOUNT", 10.0
    )
    group_id = uuid.uuid4()
    ...
    member = GroupMember(
        group_id=group_id, user_id=user_id_to_remove, contributed_amount=50.0
    )
    mock_group_repo.get_group_members.return_value = [member]

    # Mock user to remove
    mock_user_repo.get_by_id.return_value = User(
        id=user_id_to_remove, email="remove@example.com"
    )
...
@pytest.mark.asyncio
async def test_contribute_to_group_min_members(
    group_service,
    mock_group_repo,
    mock_wallet_repo,
    current_user,
    ...
    assert exc.value.status_code == status.HTTP_400_BAD_REQUEST
    assert (
        "At least 2 members" in exc.value.detail
        or "at least 2 members" in exc.value.detail
    )


@pytest.mark.asyncio
async def test_contribute_to_group_target_reached(
    group_service,
    mock_group_repo,
    mock_wallet_repo,
    current_user,

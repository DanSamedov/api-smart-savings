...
def upgrade() -> None:
    # Pre-create enums safely
    bind = op.get_bind()

    sa.Enum('EUR', 'USD', 'PLN', 'GBP', 'CAD', name='currency_enum').create(bind=bind, checkfirst=True)
    sa.Enum('USER', 'ADMIN', 'SUPER_ADMIN', 'DELETED_USER', name='role_enum').create(bind=bind, checkfirst=True)
    sa.Enum('DATA_EXPORT', 'DATA_MODIFICATION', name='gdpr_request_type_enum').create(bind=bind, checkfirst=True)
    sa.Enum('PROCESSING', 'COMPLETED', 'REFUSED', name='gdpr_request_status_enum').create(bind=bind, checkfirst=True)
    sa.Enum('ADMIN', 'MEMBER', name='grouprole').create(bind=bind, checkfirst=True)
    sa.Enum(
        'WALLET_DEPOSIT',
        'WALLET_WITHDRAWAL',
    ...
    op.create_table(
        ...
        sa.Column('role', sa.Enum(name='role_enum'), server_default='USER', nullable=False),
        ...
        sa.Column('preferred_currency', sa.Enum(name='currency_enum'), server_default='EUR', nullable=False),
        sa.Column('preferred_language', sqlmodel.sql.sqltypes.AutoString(), nullable=True),
        sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_app_user_created_at'), 'app_user', ['created_at'])
    ...
    op.create_table(
        ...
        sa.Column('currency', sa.Enum(name='currency_enum'), nullable=False),
        sa.Column('id', sa.Uuid(), nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_table(
        ...
        sa.Column('request_type', sa.Enum(name='gdpr_request_type_enum'), nullable=True),
        sa.Column('status', sa.Enum(name='gdpr_request_status_enum'), server_default='PROCESSING', nullable=False),
    ...
    op.create_index(op.f('ix_gdpr_request_created_at'), 'gdpr_request', ['created_at'])

    op.create_table(
        'group_member',
        sa.Column('role', sa.Enum(name='grouprole'), nullable=True),
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum(name='transactiontype'), nullable=True),
        sa.Column('id', sa.Uuid(), nullable=False),
        sa.Column('group_id', sa.Uuid(), nullable=False),
        sa.Column('user_id', sa.Uuid(), nullable=False),
    ...
    op.create_index(op.f('ix_wallet_created_at'), 'wallet', ['created_at'])
    ...
    op.create_table(
        ...
        sa.Column('type', sa.Enum(name='transaction_type_enum'), nullable=False),
        sa.Column('description', sqlmodel.sql.sqltypes.AutoString(), nullable=True),
        sa.Column('status', sa.Enum(name='transaction_status_enum'), server_default='PENDING', nullable=False),
        sa.Column('created_at', sa.DateTime(timezone=True), server_default=sa.text('now()'), nullable=False),
    ...
    op.create_index(op.f('ix_transaction_created_at'), 'transaction', ['created_at'])
...
def downgrade() -> None:
    ...
    # Optionally drop enums (comment out if not desired)
    for enum in ['currency_enum', 'role_enum', 'gdpr_request_type_enum', 'gdpr_request_status_enum', 'grouprole', 'transactiontype', 'transaction_type_enum']:

...
def upgrade() -> None:
    ...
    op.create_index(
        op.f("ix_app_user_created_at"), "app_user", ["created_at"], unique=False
    )
    op.create_index(
        op.f("ix_gdpr_request_created_at"), "gdpr_request", ["created_at"], unique=False
    )
    ...
    op.execute(
        "UPDATE group_member SET contributed_amount = 0 WHERE contributed_amount IS NULL"
    )
    op.execute("UPDATE group_member SET joined_at = now() WHERE joined_at IS NULL")
    op.execute(
        "UPDATE group_transaction_message SET type = 'GROUP_SAVINGS_DEPOSIT' WHERE type IS NULL"
    )
    op.execute(
        "UPDATE group_transaction_message SET timestamp = now() WHERE timestamp IS NULL"
    )
    op.execute("UPDATE groups SET current_balance = 0 WHERE current_balance IS NULL")
    op.execute(
        "UPDATE removed_group_member SET removed_at = now() WHERE removed_at IS NULL"
    )
    op.execute(
        "UPDATE transaction SET is_anonymized = false WHERE is_anonymized IS NULL"
    )
    ...
    op.alter_column(
        ...
        existing_server_default=sa.text(
            "'GROUP_SAVINGS_DEPOSIT'::transaction_type_enum"
        ),
    )
    op.alter_column(
        "group_transaction_message",
